...
function HamburgerIcon(
  toggle: boolean,
  animation: (flip: string) => string,
  handleClick: (event: MouseEvent<HTMLDivElement>) => void,
  { color = '#fff', size = 28, style, ...props }: HamburgerIconProps = {
    color: '#fff',
    size: 28,
    style: { backgroundColor: '#808080' },
  }
) {
  const common = { opacity: 1, stroke: color, transition: 'transform 0.3s' },
    flip = `
      15% {
        opacity: 0.8;
    ...
    `,
    hamburger: Partial<CSSProperties> = {
      cursor: 'pointer',
