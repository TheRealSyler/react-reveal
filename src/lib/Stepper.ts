...
class Stepper {
  steps: Step[] = [];
  stepMap: { [key: string]: Step } = {};
  hasStarted = false;
  isTriggered = false;
  runs = 1;
  totalRuns = 0;
  trigger = 0; // TODO figure out if assigning 0 is a problem.
  head = 0; // TODO figure out if assigning 0 is a problem.
  tail = 0; // TODO figure out if assigning 0 is a problem.
  constructor() {}

  step(name: string, after = 1000) {
  ...
  next() {
    ...
    this.head++;
    if (this.head >= this.steps.length) this.head = 0;
    //console.log('head:',this.head,'runs', this.runs,'total', this.totalRuns)			;
    if (onceRevealed) window.setTimeout(this.next, this.steps[prev].after);
    else this.next();
  }
}
